...
describe("NewSessionModal", () => {
  ...
  it("renders modal content when isOpen is true", () => {
    ...
    expect(screen.getByText("Selecione um agente para iniciar a sessão:")).toBeInTheDocument();
  });
  ...
  it("displays default session option", () => {
    ...
    expect(screen.getByText("Inicia uma sessão Claude Code padrão sem agente específico")).toBeInTheDocument();
  });
  ...
  it("handles agent selection", () => {
    ...
    const agentCard = screen.getByText("TestAgent1").closest("div[class*='cursor-pointer']");
  ...
  it("handles default session selection", () => {
    ...
    const defaultSessionCard = screen.getByText("Sessão Padrão").closest("div[class*='cursor-pointer']");
  ...
  it("calls onCreateSession with default session when no agent is selected", async () => {
    ...
    render(<NewSessionModal {...defaultProps} onCreateSession={mockOnCreateSession} />);
  ...
  it("shows loading state during session creation", async () => {
    const mockOnCreateSession = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
    render(<NewSessionModal {...defaultProps} onCreateSession={mockOnCreateSession} />);
  ...
  it("displays message when no agents are found", () => {
    ...
    expect(screen.getByText("Nenhum agente encontrado no workspace")).toBeInTheDocument();
  });
