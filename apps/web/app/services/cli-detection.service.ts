...
class CliDetectionService {
  ...
  async checkClaudeCodeAvailability(
    ...
    try {
      ...
      logger.error(
        "Failed to check Claude Code CLI availability",
        error as Error
      );
  ...
  private async checkWhichClaude(
    workingDirectory?: string
  ): Promise<CliDetectionResult> {
    try {
      ...
      logger.debug("which claude failed", {
        error:
          error instanceof Error
            ? this.sanitizeOutput(error.message)
            : "Unknown error",
      });
  ...
  private async getClaudeVersion(
    workingDirectory?: string
  ): Promise<CliDetectionResult> {
    try {
      const options = workingDirectory ? { cwd: workingDirectory } : {};
      ...
      const errorMessage =
        error instanceof Error
          ? this.sanitizeOutput(error.message)
          : "Unknown error";
      logger.warn("Failed to get Claude Code CLI version", {
        error: errorMessage,
      });
