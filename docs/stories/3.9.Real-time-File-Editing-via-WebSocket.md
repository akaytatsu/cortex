...
### File List
- `/apps/web/app/services/websocket-file.service.ts` - NEW: WebSocket file operations service
- `/apps/web/app/services/websocket-file.service.test.ts` - NEW: Unit tests for WebSocket service
- `/apps/web/app/lib/websocket-file-server.ts` - NEW: WebSocket server for file operations
- `/apps/web/app/routes/api.file-websocket-port.ts` - NEW: API route for WebSocket port
- `/apps/web/app/hooks/useFileWebSocket.ts` - NEW: Custom hook for file WebSocket connection
- `/apps/web/app/hooks/useDebounce.ts` - NEW: Debounce and throttling utility hooks
- `/apps/web/app/contexts/FileWebSocketContext.tsx` - NEW: React context for file WebSocket
- `/apps/web/app/components/ConnectionStatus.tsx` - NEW: Connection status indicator component
- `/apps/web/app/components/CodeViewer.tsx` - MODIFIED: Added WebSocket support with HTTP fallback
- `/apps/web/app/components/IDELayout.tsx` - MODIFIED: Wrapped CodeViewer with WebSocket provider
- `/packages/shared-types/index.ts` - MODIFIED: Added WebSocket message types and interfaces
