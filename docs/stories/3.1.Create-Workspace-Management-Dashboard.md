...
- [ ] Task 1 (AC: 1)
  - [ ] Atualizar loader da rota `/login` para redirecionar para `/workspaces` após login bem-sucedido
  - [ ] Verificar se redirecionamento já existe ou precisa ser implementado
- [ ] Task 2 (AC: 2, 3, 4, 5)
  - [ ] Criar `WorkspaceService` para gerenciar operações CRUD de workspaces no arquivo `workspaces.yaml`
  - [ ] Implementar métodos `listWorkspaces()` e `addWorkspace()` no service
  - [ ] Criar rota `/workspaces` no Remix com loader para carregar lista de workspaces
- [ ] Task 3 (AC: 2, 5)
  - [ ] Criar componente React `WorkspaceList` para exibir lista de workspaces
  - [ ] Adicionar botões "Entrar" para cada workspace (navegação para rota futura do workspace)
  - [ ] Implementar layout responsivo usando Tailwind CSS e shadcn/ui
- [ ] Task 4 (AC: 3, 4)
  - [ ] Criar componente `EmptyWorkspaces` para estado vazio da lista
  - [ ] Adicionar botão "Adicionar Workspace" que navega para formulário de adição
  - [ ] Implementar mensagens instrucionais para o usuário
- [ ] Task 5 (AC: 4)
  - [ ] Criar rota `/workspaces/new` para formulário de adição de workspace
  - [ ] Criar componente `AddWorkspaceForm` usando padrão estabelecido (similar a LoginForm/SetupForm)
  - [ ] Implementar action para processar criação de novo workspace
- [ ] Task 6: Testing
  - [ ] Adicionar testes unitários para `WorkspaceService`
  - [ ] Criar testes de integração para rotas `/workspaces` e `/workspaces/new`
  - [ ] Testar componentes `WorkspaceList`, `EmptyWorkspaces` e `AddWorkspaceForm`
  - [ ] Testar fluxos de navegação e redirecionamentos
